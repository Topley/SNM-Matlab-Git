...
Muscles = {'TA';'Sol';'MG';'LG'};

fileStruct = load(fullfile(fileDir,filename));%, 'MUPulses');
displayPulses = fileStruct.MUPulses;
TraceFeedback = fileStruct.TraceFeedback;
...
try
    EMGFeedback = filterEMG(fileStruct.EMG); 
catch
    try
        EMGFeedback = filterEMG(fileStruct.EMG); 
    catch
        EMGFeedback = fileStruct.EMGFeedback;
    end
end

%%% Find the contraction hold window based on the EMG activity of muscle
%%% used for visual feedback
[stax] = findHolds(TraceFeedback,EMGFeedback, EMG, coco)
...
if contains(filename,'TA') && coco > 0 
    try
        ...
        fileStructM2 = load(fullfile(fileDir,m2Filename));
    ...
    try
        ...
        fileStructM3 = load(fullfile(fileDir,m3Filename));
...
if numberMuscles == 2
    try
        ...
        m2STFigure = m2ST;
        try
            tempEMG2 = load(fullfile(fileDir,m2Filename), 'EMG');
            m2RawEMG = abs(sum(tempEMG2.EMG));
            m2RmsEMG = rms(m2RawEMG,fsamp);
            m2EMGFeedback = movmean(m2RmsEMG,fsamp*2);
        catch
            try
                tempEMG2 = load(fullfile(fileDir,m2Filename), 'EMGall');
                m2RawEMG = abs(sum(tempEMG2.EMGall));
                m2RmsEMG = rms(m2RawEMG,fsamp);
                m2EMGFeedback = movmean(m2RmsEMG,fsamp*2);
            catch
                tempEMG2 = load(fullfile(fileDir,m2Filename), 'EMGFeedback');
                m2EMGFeedback = tempEMG2.EMGFeedback;
            end
        end
        
        [~,holdIndexes2]  = maxk(abs(diff(m2EMGFeedback(1:fsamp*5:end))),2);
        staxM2 = (min(holdIndexes2)*5)+5;
        endaxM2 = (max(holdIndexes2)*5)-5;
        holdWindow2 = endaxM2-staxM2;
...
if numberMuscles == 3
    try
        ...
        m3STFigure = m3ST;
        try
            tempEMG3 = load(fullfile(fileDir,m3Filename), 'EMG');
            m3RawEMG = abs(sum(tempEMG3.EMG));
            m3RmsEMG = rms(m3RawEMG,fsamp);
            m3EMGFeedback = movmean(m3RmsEMG,fsamp*2);
        catch
            try
                tempEMG3 = load(fullfile(fileDir,m3Filename), 'EMGall');
                m3RawEMG = abs(sum(tempEMG3.EMGall));
                m3RmsEMG = rms(m3RawEMG,fsamp);
                m3EMGFeedback = movmean(m3RmsEMG,fsamp*2);
            catch
                tempEMG3 = load(fullfile(fileDir,m3Filename), 'EMGFeedback');
                m3EMGFeedback = tempEMG3.EMGFeedback;
            end
        end
        
        [~,holdIndexes3]  = maxk(abs(diff(m3EMGFeedback(1:fsamp*5:end))),2);
        staxM3 = (min(holdIndexes3)*5)+5;
        endaxM3 = (max(holdIndexes3)*5)-5;
        holdWindow3 = endaxM3-staxM3;
...
[m1goodUnits,m1goodFiring, m1COV, rSpike1] = removeBUsCoherenceAuto(filename, displayMotorUnits,displayST,[stax endax]);
displaySTFigure(rSpike1,:) = [];

%%%% Plot other muscles spike trains and remove bad ones & calculate COV
if numberMuscles >= 2
    try
        [m2goodUnits,m2goodFiring, m2COV, rSpike2] = removeBUsCoherenceAuto(m2Filename, m2MotorUnits,m2ST,[stax endax]);
        m2STFigure(rSpike2,:) = [];
    catch
...
if numberMuscles == 3
    try
        [m3goodUnits,m3goodFiring, m3COV, rSpike3] = removeBUsCoherenceAuto(m3Filename, m3MotorUnits,m3ST,[stax endax]);
        m3STFigure(rSpike3,:) = [];
    catch
...
%     hold on
    plot([0:length(displaySTFigure)-1]/fsamp,fftfilt(hanning(fsamp),displaySTFigure'));
    plot([0:length(displaySTFigure)-1]/fsamp,mean(fftfilt(hanning(fsamp),displaySTFigure'),2),'Linewidth',3,'Color','k');
%     
    if numberMuscles >= 2
        try
...
try
    ...
    subfiring = displayST(:,stax*fsamp:endax*fsamp);
end
...
if contains(filename, 'TA')
    ...
    print (figure(101),'-dpdf',[filename_save '.pdf']);
...
function [poolFig,cstCOHT, poolZ, cstF, pF] = pooledInterCoherence(muscle1, muscle2, muscle1Firing,muscle2Firing,LW,fsamp)
% This function will take the smoothed binary spike trains of the active
