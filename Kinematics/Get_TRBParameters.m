...
while stopLoop ~= 0
    ...
    groupInfo = fread(fileID, 2, 'int16', 'ieee-le');
    
    c3dStruct.Parameters.GroupID = [c3dStruct.Parameters.GroupID;groupInfo(2)];
    stopLoop = groupInfo(2);
    
    binaryParamName = fread(fileID, abs(groupInfo(1)), 'int16', 'ieee-le');
    ...
    elseif stopLoop >= 0
        dimData = fread(fileID, 2, 'int16', 'ieee-le');
        dataType = dimData(1);
        numDim = dimData(2);
        ...
        paramStartDim = fread(fileID, numDim, 'int16', 'ieee-le');
    ...
    descriptSize = fread(fileID, 1, 'int16', 'ieee-le');
    c3dStruct.Parameters.DescriptionSize = descriptSize(1);
    groupParamDescript = fread(fileID, descriptSize(1), 'int16', 'ieee-le');
