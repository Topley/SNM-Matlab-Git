...
function [rere, dere] = delta_f_analysis_TRD_v1(fullFileName, usermaxTorque, Auto, saveexcel, keepPics, ramp)
...
[fileDir,cleanedFile,decompFile, clusterFile, pdfdir] = Set_Directories(fullFileName, 'ramp');

% Open Decomposed MU File
temp = matfile(fullfile(fileDir, cleanedFile));
MUPulses = temp.MUPulses;
% Open Variables from Cluster File
cVars = matfile(fullfile(fileDir,clusterFile));
fsamp = cVars.fsamp;
EMG = cVars.EMG;
TorqueFeedback = cVars.AnalogData(12,:);
Torque = cVars.AnalogData(11,:);
TraceFeedback = Torque;
...
if contains(cleanedFile,'TA')
    try
        [SolEMGFeedback] = findAntagonistEMG(fullfile(fileDir,clusterFile), 'TA', 'Sol');
        [MGEMGFeedback] = findAntagonistEMG(fullfile(fileDir,clusterFile), 'TA', 'MG');
    catch
        MGEMGFeedback = [];
        SolEMGFeedback = [];
    end
elseif contains(cleanedFile,'Sol')
    try
        [TAEMGFeedback] = findAntagonistEMG(fullfile(fileDir,clusterFile), 'Sol', 'TA');
        [MGEMGFeedback] = findAntagonistEMG(fullfile(fileDir,clusterFile), 'Sol', 'MG');
    catch
        MGEMGFeedback = [];
        TAEMGFeedback = [];
    end
elseif contains(cleanedFile,'MG')
    try
        [TAEMGFeedback] = findAntagonistEMG(fullfile(fileDir,clusterFile), 'MG', 'TA');
        [SolEMGFeedback] = findAntagonistEMG(fullfile(fileDir,clusterFile), 'MG', 'Sol');
    catch
        TAEMGFeedback = [];
        SolEMGFeedback = [];
    end
...
%%%%%
if Auto == 1
    [Trialcut] = findRamps(TraceFeedback,EMGFeedback,EMG, coco);
...
try
    ...
    [TQTime, timeTorque, cutTorque, cutTorqueFeedback, pfTQfeedback, dfTQfeedback] = TrimTorque(MUTime,TorqueFeedback, TorqueFeedback, usermaxTorque, coco);
...
if Pics == 4
    ...
    title4All = [cleanedFile(1:end-4),'_allunits_',num2str(MUTime(1)),'_',num2str(MUTime(2)),'.pdf'];
    ...
    for j = 1:length(windowST)
        ...
        if Tdiff>0 % && min(windowST{j})<max(windowST{i})
            ...
            if or(Tdiff > 1-2 && rvalue > 0.836-2 && ratemod > 0.5-2 && i~=length(windowST),i==length(windowST)); %%%%%% changed
                ...
                output(c,:) = [TRUEMU(i),TRUEMU(j),deltaf,Tdiff,rvalue,slope,ratemod,initiali,maxi,finali,initialj,maxj,finalj,Fstart,...
                    min(windowST{i}),max(windowST{i}),min(windowST{j}),max(windowST{j}),...
                    cutTorque((min(MUFiring{i})*960-TQTime(1))),cutTorque((max(MUFiring{i})*960-TQTime(1))),...
                    cutTorque((min(MUFiring{j})*960-TQTime(1))),cutTorque((max(MUFiring{j})*960-TQTime(1))),...
                    newEMGFeedback((min(MUFiring{i})*960-TQTime(1))),newEMGFeedback((max(MUFiring{i})*960-TQTime(1))),...
                    newEMGFeedback(min(MUFiring{j})*960-TQTime(1)),newEMGFeedback((max(MUFiring{j})*960-TQTime(1)))];%%%%%%%%%%
                c=c+1;
            end
...
if Pics >= 2 % IF u want PDFs of individual units displayed
    ...
    for i = 1:length(MUFiring)
        ...
        title({cleanedFile(1:end-4), ['Unit ', num2str(TRUEMU(i), '%02i')]}, 'Interpreter', 'none');
        axis(plot_axis)
        
        if keepPics >= 2
            savenamef99 = [cleanedFile(1:end-4),'_Unit_', num2str(TRUEMU(i), '%02i'),'_',num2str(MUTime(1)),'_',num2str(MUTime(2))];
...
if Pics >= 1
    ...
    for i = 1:length(windowDR)
        ...
        title({[clusterFile(1:end-4),'_',num2str(MUTime(1)),'_',num2str(MUTime(2))],['All ',num2str(length(MUFiring)),' units']},'interpreter','none')
    end
    ...
    if keepPics >= 1
        ...
        if ramp == 1
            sumFigTitle = [cleanedFile(1:end-4),'_',num2str(MUTime(1)),'_',num2str(MUTime(2)),'_Ramp1.pdf'];
        elseif ramp == 2
            sumFigTitle = [cleanedFile(1:end-4),'_',num2str(MUTime(1)),'_',num2str(MUTime(2)),'_Ramp2.pdf'];
        elseif ramp == 3
            sumFigTitle = [cleanedFile(1:end-4),'_',num2str(MUTime(1)),'_',num2str(MUTime(2)),'_Ramp3.pdf'];
        end
...
if saveexcel == 1
    ...
    %     if MUTime(1) < 20
    dlmwrite(fullfile(exceldir,[cleanedFile(1:end-4),'_',num2str(MUTime(1)),'_',num2str(MUTime(2)),'_deltaf.xls']),output,'delimiter','\t')
    %         elseif MUTime(1) < 60
